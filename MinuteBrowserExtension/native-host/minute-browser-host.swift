...
func readMessage() -> Data? {
    ...
    // Log start to workspace for debugging
    let logMsg = "Host started at \(Date())\n"
    if let data = logMsg.data(using: .utf8) {
        // Try creating file first if needed
        if !FileManager.default.fileExists(atPath: "/Users/tycho/Projects/Minute/debug_host.log") {
            FileManager.default.createFile(atPath: "/Users/tycho/Projects/Minute/debug_host.log", contents: nil)
        }
        
        if let handle = FileHandle(forWritingAtPath: "/Users/tycho/Projects/Minute/debug_host.log") {
